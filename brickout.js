...
function settingStage(e) {
    ...
    /* stage 상대 좌표 이용하여 선택하기
    
        var relativeX = (e.clientX - canvas.offsetLeft) * WIDTH / canvas.clientWidth;
        var relativeY = e.clientY * HEIGHT / canvas.clientHeight;
    
        // stage 1 선택
        if(isInBorder(0, 75, relativeX) && isInBorder(115, HEIGHT, relativeY)) test(1);
     */
...
function move_settingPage(e) {
    if ((e.clientX >= 1090 && e.clientX <= 1204) && (e.clientY >= 529 && e.clientY <= 603)) { settingPage(); }
}
